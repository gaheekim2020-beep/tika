...
### 1.4 프론트엔드 컴포넌트 경계 (서버/클라이언트 컴포넌트)

Next.js App Router는 컴포넌트를 기본적으로 **서버 컴포넌트**로 렌더링하며, 상태(`useState`)·이펙트(`useEffect`)·이벤트 핸들러·브라우저 API를 사용하는 컴포넌트만 파일 최상단에 `"use client"`를 선언해 **클라이언트 컴포넌트**로 전환한다. Tika는 이 경계를 다음 규칙으로 고정한다.

| 규칙 | 내용 |
|------|------|
| 진입점은 서버 컴포넌트 | `app/page.tsx`(및 각 라우트의 `page.tsx`)는 `"use client"` 없는 서버 컴포넌트로 유지한다. 데이터 페칭·상태·인터랙션이 필요한 하위 트리는 별도 클라이언트 컴포넌트로 위임하고, 페이지 자체는 그 컴포넌트를 렌더링하는 진입점 역할만 한다. |
| 클라이언트 경계 최소화 | `"use client"`는 실제로 상태/이벤트/DnD가 필요한 최상위 컨테이너 컴포넌트 하나에만 선언한다 (예: 보드 화면의 `BoardContainer`). 그 하위 컴포넌트들은 별도로 `"use client"`를 반복 선언하지 않고, 부모의 클라이언트 경계에 자연스럽게 포함된다. |
| 클라이언트 컴포넌트가 필요한 조건 | 다음 중 하나라도 해당하면 클라이언트 컴포넌트다: (1) `useState`/`useEffect` 등 React 훅으로 상태를 관리, (2) `onClick`/`onDragEnd` 등 이벤트 핸들러를 직접 등록, (3) `@dnd-kit/core`의 `DndContext` 등 브라우저 전용 라이브러리를 사용. |
| 명세 문서 표기 | COMPONENT_SPEC.md의 컴포넌트 계층 트리는 서버 컴포넌트와 클라이언트 컴포넌트 경계가 시작되는 지점을 주석으로 명시한다 (COMPONENT_SPEC.md §1 참조). |

### 1.5 드래그 앤 드롭 구조 (@dnd-kit)

`@dnd-kit/core` + `@dnd-kit/sortable`를 다음 3개 컴포넌트 조합으로 사용한다. COMPONENT_SPEC.md의 컴포넌트 트리는 이 구조를 그대로 반영한다.
